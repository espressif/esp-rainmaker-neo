...
def _test_push_e2e_set_mock_random(value):
    """Point the notifications lambda's mock_random at this test's UUID (empty string to reset)."""
    subprocess.run([
        sys.executable, "tools/drx.py", "update-env",
        "rmng-notifications",
        f"mock_random={value}"
...
def _test_push_e2e_connect_and_trigger(device, group_id):
    """Connect the device, configure a test light, and fire a push-notification shadow update."""
    assert device.connect(), "Failed to connect the device"
    device.set_node_config({
        "devices": [{
            "id": "TestLight",
    ...
    if hasattr(device, 'subgroup_ids') and device.subgroup_ids:
        for subgroup_id in sorted(device.subgroup_ids):
            shadow_name += f"-{subgroup_id}"
    assert device.shadow_connect([shadow_name]), "Failed to connect to shadow"
    print(f"✅ Device connected and shadow ready: {shadow_name}")

    device.update_named_shadow(shadow_name, {"TestLight": {"Power": False, "Brightness": 30}})
    time.sleep(2)
    device.update_named_shadow(shadow_name, {
        "TestLight": {"Power": True, "Brightness": 80},
        "notify": {"version": random.randint(1, 1000000), "push": True},
    })
    print("🔔 Push notification triggered! Waiting for the phone to receive and forward...")
    return shadow_name


def _push_e2e_poll_validate(uuid, timeout=10, interval=2):
    """Poll the webhook mock for a forwarded notification keyed by uuid. Real-device delivery latency varies, so retry rather than sleep once."""
    # Still targets vercel (not the in-cloud mock): the forwarder here is the lab phone's compiled NotificationTest app, hardcoded to this URL, so it can't be repointed from this repo — unlike the server-side report-state path the mock migration covered.
    deadline = time.time() + timeout
    last = None
    while time.time() < deadline:
        last = requests.get("https://webhook-mock.vercel.app/api/validate", params={"uuid": uuid})
...
@pytest.mark.xdist_group("env_mut")
def test_push_notification_ios_end_to_end(
    associated_device, super_admin_user, apns_credentials, apns_device_token
):
    """End-to-end iOS (APNS) push: register a sandbox platform + device token, trigger a shadow update, and assert the lab phone forwarded the notification to the webhook mock.

    The lab iOS app forwards received notifications to the mock; it is the
    `NotificationTest` sample from the ESP app SDK.
    """
    ios_config = apns_credentials
    ios_device_token = apns_device_token

    device, group_id, test_user1, user1_group_api = associated_device

    test_random = str(uuid.uuid4())
    print(f"Configuring test UUID: {test_random}")
    _test_push_e2e_set_mock_random(test_random)

    ios_register_result = None
    try:
        # Lab phone runs a dev build → its token is a sandbox token; the production gateway rejects it with BadDeviceToken. Register against sandbox.
        ...
        assert ios_result is not None, "Failed to create iOS platform application"
        print(f"✅ iOS integration created: {ios_result['integration_id']}")

        platforms_list = super_admin_user.list_mobile_platforms()
        assert platforms_list and "integrations" in platforms_list, "Failed to get platforms list"
        integration_ids = [p["integration_id"] for p in platforms_list["integrations"]]
        assert f"apns_sandbox_{ios_config['bundle_id']}" in integration_ids, f"iOS integration not found in list: {integration_ids}"

        ios_register_result = test_user1.register_client(
            platform_type="APNS_SANDBOX",
            platform_app_name=ios_config["bundle_id"],
            mobile_device_token=ios_device_token,
            locale="en_US"
        )
        assert ios_register_result, "Failed to register iOS device token"
        print(f"✅ iOS device token registered: {ios_device_token}")

        _test_push_e2e_connect_and_trigger(device, group_id)

        response = _push_e2e_poll_validate(f"{test_random}_APNS")
        print(f"🔍 APNS uuid={test_random}_APNS Response: {response.text}")
        assert response.status_code == 200, f"Failed to validate iOS push notification: {response.text}"

        ios_response_data = response.json()
        ios_response_data["data"]["event_data"]["ts"] = 0
        expected_ios_format = {
            "data": {
                "aps": {
                    "mutable-content": 1,
                    "alert": {
                        "title": "Node Alert",
                        "body": f"Node {device.node_thing_name} has an alert!"
                    },
                    "category": "node_alert",
                    "sound": "default",
                    "thread-id": device.node_thing_name + ".node.alert",
                ...
                "event_data": {
                    "data": {"nodeID": device.node_thing_name},
                    "ts": 0,
                    "type": "node_alert",
                    "uuid": f"{test_random}_APNS",
                }
            }
        }
        assert ios_response_data == expected_ios_format, f"iOS notification format mismatch.\nExpected: {expected_ios_format}\nGot: {ios_response_data}"
        print("✅ iOS push notification validated with correct format")
    finally:
        print("🧹 Cleaning up iOS resources...")
        if ios_register_result:
            unregister_response = test_user1.unregister_client(
                "APNS_SANDBOX", ios_register_result, ios_config["bundle_id"]
            )
            assert unregister_response.status_code != 500, (
                f"APNS_SANDBOX unregister returned 500 — endpoint leaked, notifications will "
                f"continue after logout: {unregister_response.text}"
            )
            print("   ✅ Unregistered APNS_SANDBOX endpoint")
        try:
            super_admin_user.delete_mobile_platform("apns_sandbox", ios_config["bundle_id"])
            print("   ✅ Deleted iOS integration")
        except Exception as e:
            print(f"   ⚠️ Failed to delete iOS integration: {e}")
        try:
            device.disconnect()
        except Exception as e:
            print(f"   ⚠️ Failed to disconnect device: {e}")
        _test_push_e2e_set_mock_random("")
        print("🧹 Cleanup completed!")


@pytest.mark.xdist_group("env_mut")
def test_push_notification_android_end_to_end(
    associated_device, super_admin_user, firebase_service_account, firebase_device_token
):
    """End-to-end Android (FCM/GCM) push: register a platform + device token, trigger a shadow update, and assert the lab phone forwarded the notification to the webhook mock.

    The lab Android app forwards received notifications to the mock; it is the
    `NotificationTest` sample from the ESP app SDK.
    """
    android_config = firebase_service_account
    android_device_token = firebase_device_token
    ...
    android_register_result = None
    try:
        android_result = super_admin_user.register_android_platform(
            json_content=json.dumps(android_config)  # Firebase service account JSON as string
        )
        assert android_result is not None, "Failed to create Android platform application"
        print(f"✅ Android integration created: {android_result['integration_id']}")

        platforms_list = super_admin_user.list_mobile_platforms()
        assert platforms_list and "integrations" in platforms_list, "Failed to get platforms list"
        integration_ids = [p["integration_id"] for p in platforms_list["integrations"]]
        assert f"gcm_{android_config['project_id']}" in integration_ids, f"Android integration not found in list: {integration_ids}"

        android_register_result = test_user1.register_client(
            platform_type="GCM",
            platform_app_name=android_config["project_id"],
            mobile_device_token=android_device_token,
            locale="es_ES"
        )
        assert android_register_result, "Failed to register Android device token"
        print(f"✅ Android device token registered: {android_device_token}")

        _test_push_e2e_connect_and_trigger(device, group_id)

        response = _push_e2e_poll_validate(f"{test_random}_GCM")
        print(f"🔍 GCM uuid={test_random}_GCM Response: {response.text}")
        assert response.status_code == 200, f"Failed to validate Android push notification: {response.text}"

        android_response_data = response.json()
        android_response_data["data"]["event_data"]["ts"] = 0
        expected_android_format = {
            "data": {
                "body": f"Node {device.node_thing_name} has an alert!",
                "title": "Node Alert",
                # It seems the android priority field is not visible to the phone app
                "event_data": {
                    "data": {"nodeID": device.node_thing_name},
                    "ts": 0,
                    "type": "node_alert",
                    "notif_grp_id": device.node_thing_name + ".node.alert",
                    "uuid": f"{test_random}_GCM"
                },
            }
        }
        assert android_response_data == expected_android_format, f"Android notification format mismatch.\nExpected: {expected_android_format}\nGot: {android_response_data}"
        print("✅ Android push notification validated with correct format")
    finally:
        print("🧹 Cleaning up Android resources...")
        if android_register_result:
            unregister_response = test_user1.unregister_client(
                "GCM", android_register_result, android_config["project_id"]
            )
            assert unregister_response.status_code != 500, (
                f"GCM unregister returned 500 — endpoint leaked, notifications will continue "
                f"after logout: {unregister_response.text}"
            )
            print("   ✅ Unregistered GCM endpoint")
        try:
            super_admin_user.delete_mobile_platform("GCM", android_config["project_id"])
            print("   ✅ Deleted Android integration")
        except Exception as e:
            print(f"   ⚠️ Failed to delete Android integration: {e}")
        try:
            device.disconnect()
        except Exception as e:
            print(f"   ⚠️ Failed to disconnect device: {e}")
