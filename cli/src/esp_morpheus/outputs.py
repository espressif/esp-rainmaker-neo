...
# Kept for the in-repo callers that still import it; None outside a checkout.
REPO_ROOT = str(paths.repo_root()) if paths.repo_root() else None
...
TEST_CONFIG_PATH = str(paths.test_config_path())
...
class OutputsError(SystemExit):
    """A deployment that cannot be acted on: unreadable outputs, or credentials for another account.

    @note A SystemExit, so a plain script still exits with the message; the CLI catches it by name
    and reports it as a command failure instead of ending the session.
    """
...
def resolve_source(source=None):
    ...
    Inside a checkout, relative paths anchor to the repo root so the tool behaves identically from
    any directory in it; installed, they anchor to the CWD. URLs and absolute paths pass through.
    """
    ...
    return os.path.join(root, source) if root else os.path.abspath(source)
...
def verify_aws_identity(settings):
    """Abort unless the ambient AWS credentials point at the account the outputs describe.

    Called by the callers that reach AWS directly (IoT, Lambda, DynamoDB, Cognito, IAM), where a
    mismatch does not fail loudly — it silently reads and mutates a different deployment than the
    one requested. Driving a deployment through its own API needs no credentials and no check.

    There is deliberately no way to skip this. Every command that calls it creates, deletes or
    sweeps real resources, which is exactly where writing to the wrong account is unrecoverable.
    """
    ...
    try:
        ...
        raise OutputsError(
            "no AWS credentials found. This command reaches AWS resources directly; "
    ...
    if caller['Account'] != settings.account_id:
        raise OutputsError(
            f"AWS account mismatch. {settings.source} describes account {settings.account_id}, "
            f"but the configured credentials are for {caller['Account']} ({caller['Arn']}). Point "
            "--client-outputs at the matching deployment, or switch AWS profile."
        )

    # The region is reported, not enforced: every client is built with region_name from the
    # outputs, so the ambient region cannot change which deployment is touched. Aborting on it
    # would reject a harmless mismatch, and the escape hatch for that used to be a flag that also
    # switched off the account check above, which is the one that matters.
    ambient_region = os.environ.get('AWS_REGION') or boto3.session.Session().region_name
    if ambient_region != settings.region:
        described = f"the environment resolves to {ambient_region}" if ambient_region \
            else "no region is configured"
        print(f"Note: {settings.source} describes region {settings.region} and {described}; "
              f"using {settings.region}.")
