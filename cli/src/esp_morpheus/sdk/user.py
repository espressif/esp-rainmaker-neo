...
# Off by default: the trace prints bearer tokens and STS session credentials verbatim, so it
# belongs on a terminal only when the operator asks for it. `morpheus -v` turns it on.
request_logging = False


def trace_request(method, url, body=None):
    """Print a request verbatim. Bodies are not filtered: a token you cannot read is a token you
    cannot check the claims of, which is the reason to turn the trace on."""
    if request_logging:
        user_log(f"-> {method} {url}" + (f" {body}" if body else ''))


def trace_response(response):
    if request_logging:
        user_log(f"<- {response.status_code} {response.text}")
...
def _admin_cognito_auth(region, client_id, auth_flow, auth_parameters):
    ...
    initiate_auth is an unauthenticated Cognito operation — the client id and the password are the
    whole credential — so the call is made unsigned when the caller has no AWS credentials. Signing
    it otherwise keeps the request identical to what the tests and CI already send.
    """
    try:
        signed = boto3.Session(region_name=region).get_credentials() is not None
    except Exception:  # noqa: BLE001 - an unreadable credential chain is simply "none"
        signed = False
    client = boto3.client('cognito-idp', region_name=region) if signed else boto3.client(
        'cognito-idp', region_name=region, config=BotocoreConfig(signature_version=UNSIGNED))
    try:
...
class User:
    ...
    def make_user_api_request(self, method, path, data=None, params=None, require_token=True, token=None):
        ...
        if require_token:
            ...
            trace_request(method, url, json_data)
            response = self.session.request(
                method=method,
                url=url,
                headers=headers,
